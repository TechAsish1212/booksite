    ...
    <div className='mx-auto max-w-[1440px] flex flex-col sm:flex-row'>
      ...
      <div className='max-sm:flexCenter max-sm:pb-3 bg-primary pb-3 m-2 sm:min-w-[20%] sm:min-h-97vh] rounded-xl'>
        <div className='flex flex-col gap-y-6 max-sm:items-center sm:flex-col pt-4 sm:pt-14'>
          {/* logo */}
          <Link to={'/admin'} className='bold-28 md:bold-24 uppercase font-paci lg:pl-[15%]'>Ink<span className='text-secondary bold-28'>Well.</span></Link>
          <div className='flex sm:flex-col sm:gap-x-5 gap-y-8 sm:pt-10'>
            {navItems.map((link) => (
              <NavLink key={link.label} className={({isActive})=>isActive?"flexStart gap-x-2 p-5 lg:pl-12 medium-15 cursor-pointer h-10 text-secondary bg-white max-sm:border-b-4 sm:border-r-4 border-secondary":"flexStart gap-x-2 lg:pl-12 p-5 medium-15 cursor-pointer h-10 rounded-xl"} to={link.path} end={link.path === '/admin'}>
                {link.icon}
