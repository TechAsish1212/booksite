...
const Order = () => {
  ...
  return (
    <div className='px-2 sm:px-6 py-12 m-2 h-[97vh] bg-primary overflow-y-scroll lg:w-4/5 rounded-xl'>
      {orders.map((order) => (
        <div key={order._id} className='mt-4 p-2 bg-white rounded-md'>
          {/* book list */}
          <div className='flex flex-col lg:flex-row gagp-4 mb-3'>
            {order.items.map((item, ind) => (
              <div key={ind} className='flex gap-x-4'>
                <div className='flexCenter rounded-lg overflow-hidden'>
                  <img src={item.book.image[0]} alt="oderImg" className='max-h-20 max-w-32 aspect-square object-contain' />
                </div>
                <div className='w-full block'>
                  <h5 className='h5 capitalize line-clamp-1'>{item.book.name}</h5>
                  <div className='flex felx-wrap gap-3 max-sm:gap-y-1 mt-1'>
                    <div className='flex items-center gap-x-2'>
                      <h5 className='h5 medium-14'>Price:</h5>
                      <p>{currency}{item.book.offeredPrice}</p>
                    </div>
                    <div className='flex items-center gap-x-2'>
                      <h5 className='h5 medium-14'>Quantity:</h5>
                      <p>{item.quantity}</p>
                    </div>
                  </div>
                </div>
          ...
          <div className='flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-t border-gray-300 pt-3'>
            <div className='flex flex-col gap-3'>
              ...
              <div className='flex gap-4 '>
                <div className='flex items-center gap-x-2'>
                  <h5 className='h5 medium-14'>Customer:</h5>
                  <p className='text-xs'>{order.address.firstName} {order.address.lastName}</p>
                </div>
                <div className='flex items-center gap-x-2'>
                  <h5 className='h5 medium-14'>Phone:</h5>
                  <p className='text-xs'>{order.address.phone}</p>
                </div>
              </div>

              <div className='flex items-center gap-x-2'>
                <h5 className='h5 medium-14'>Address:</h5>
                <p className='text-xs'>{order.address.street}{" "},{order.address.city}{" "},{order.address.zipcode}{" "},{order.address.country}</p>
              </div>

              <div className='flex gap-4 '>
                <div className='flex items-center gap-x-2'>
                  <h5 className='h5 medium-14'>Payment Status:</h5>
                  <p>{order.isPaid ? "Done" : "Pending"}</p>
                  <div className='flex items-center gap-x-2'>
                    <h5 className='h5 medium-14'>Payment Method:</h5>
